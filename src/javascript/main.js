...
let apiKey = '168845011d00a3';
json(`https://ipinfo.io?token=${apiKey}`).then(data => {
ip = data.ip;
city = data.city;
...
loc = data.loc;
 /* console.log(data.city);
    ...
    console.log(ip);*/
var datelogin2 = moment.tz(timezone).format('MMMM Do YYYY, h:mm:ss a');
var State2 = "<img src='./src/images/being.png' id='stateimg'>";
scrollToBottom = () => {
  $('body').scrollTo('100%', { duration: 1000,background:"blue" })
}
...
function sendMessage(e) {
  e.preventDefault();
  var localTimestamp = moment.tz(timezone).format("x");
...
Toast.fire({
 ...
 title: '<b dir="rtl">مرحبا بك'+' '+username+'</b>',
    ...
    setUsername(); // الرجوع لواجهة ادخال الاسم عند عدم وجود كوكيز
  }
} else {
...
//كود مغرفة المستخدم لديه حساب ام لا
...
var profile = `<div class='profile2'><img src='./src/images/user.png'></div>`;
    }
...
//كود معرفة المستخدم هل هو متصل ام لا
if(Cookies.get("username3")){
...
if (document.visibilityState == 'hidden') {
 ...
 $("#State").html(State2);
 var  log = "of";
 var   lastime = moment.tz(timezone).format("x");
db.ref("infouser/"+username).update({
  username,
   log,
  lastime,
...
let  localTimestamp = moment.tz(timezone).format("x");
if(username2 == ''){
swal({
title: '!خطا',
text: 'يرجي ملء اسمك.',
...
element: "input",
attributes: {
placeholder: "...رجاء اكتب  كلمة مرورك",
type: "password",
    }},
closeOnEsc: false,
closeOnClickOutside: false,
...
db.ref("infouser/"+username).once("value" , (snapshot3) => {
  ...
  let noimage = "./src/images/user.png";
   $('#profile-img').attr('value', `${image}`);
  //  console.log(image);
   if(State == "on"){
...
$('#registration-username').attr('value', `${username}`);
$('#registration-password').attr('value', `${password}`);
  if(image){
