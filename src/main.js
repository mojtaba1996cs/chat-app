...
fetchChat.on("child_added", function (data) {
  hideLoader();
  audio.play();
  try {
...
let iduser2 = messagesData.iduser;
  ...
  db.ref("infouser/"+senderName).once("value", (snapshot2) => {
    var statelog = snapshot2.val().log;
    console.log(statelog);
    var statelog2 = `<img src="./src/images/${statelog}.png" class='stateimg2 ${type}'>`;
    var images = snapshot2.val().image;
    console.log(images);
   if(images == undefined ){
     var images2 = `<div class="profile"><img src="./src/images/user3.png" alt="profile" class="home-img2 ${type}"/>${statelog2}</div>`;
   }else{
    var images2 = `<div class="profile"><img src="${images}" alt="profile" class="home-img2 ${type}"/>${statelog2}</div>`;
   }

const message = `<div class="meg">`+images2+`<div class="message ${type}"><p class="username"> <a href="inf-user.html?id=${iduser2}&name=${senderName}" class="username">${senderName} </a><spam class ="county" dir="rtl">من ${countryName + " " + countryEmoji}</spam> </p><p class="msg-text">${senderMessage}</p><p class="msg-time ${type}">${relativeSendingTime}</p></div></div>`;
//console.log(message);
   document.querySelector('.message-container').innerHTML += message;
    /*var con = document.getElementById('message-container');
Array.prototype.slice.call(con.childNodes).reverse().forEach(function(e) {
    con.appendChild(e);
});*/
  });
  }catch (error) {
